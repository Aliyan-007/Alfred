...
def record_audio(output_file="voice_input.wav"):
    ...
    while True:
        ...
        pre_buffer = deque(maxlen=int(PRE_BUFFER_DURATION / chunk_duration))
        recorded_chunks = []
        ...
        try:
            with sd.InputStream(
                ...
                for _ in range(5):
                    ...
                    noise_samples.append(get_rms(chunk))
                ...
                while True:
                    ...
                    if not speech_started:
                        ...
                        trigger_hits = trigger_hits + 1 if volume >= energy_trigger else max(0, trigger_hits - 1)
                        if trigger_hits >= 2:
                    ...
                    silence_time = silence_time + chunk_duration if volume < energy_trigger else 0.0
            ...
            print(f"\n[RECORDER ERROR] Microphone [{microphone}] failed: {error}", flush=True)
            excluded.add(microphone)
            print("[MIC] Trying another input device...", flush=True)
            continue

        if not recorded_chunks:
            return None

        sf.write(output_file, np.concatenate(recorded_chunks, axis=0), SAMPLE_RATE)
